 ...
 * 公交运行情况
 */
public class TestBusStationActivity extends BaseAppActivity {
...
private void searchBus(){
	RequestParams params = new RequestParams();
	try {
		params.put("",DESCodeUitl.getCipherString("{\"lineName\":\"528\"}"));
...
//                    Log.i(TAG, "onClientSuccess: =="+new String(DESCodeUitl.decode(Base64.decode(station.getCRYPTOGRAPH().getBytes("utf-8"), 0), "nbt_ztesoft_!@#$%^&=="), "utf-8"));
            ...
            case Api.getBusLineUrl:
                try {
                    Log.i(TAG, "onClientSuccess: =="+DESCodeUitl.getOriginString(json.toString()));
                } catch (UnsupportedEncodingException e) {
                    e.printStackTrace();
                }
                break;
    ...
    private void getBusLine(String str){
        RequestParams params = new RequestParams();
        try {
            Log.i(TAG, "getBusLine: =="+ DESCodeUitl.getCipherString(str));
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
